...
public class Student extends User {
    ...
    public boolean dropCourse(Course course, LocalDate globalDropDeadline) throws DropDeadlinePassedException {
        if (!(registeredCourses.contains(course))) {
            System.out.println("Not registered for the course.");
            return false;
        }
        LocalDate today = LocalDate.now();
        if (today.isAfter(globalDropDeadline)) {
            throw new DropDeadlinePassedException("Cannot drop the course " + course.getTitle() + ": Global drop deadline has passed.");
        }
    ...
    public <T> void submitFeedback(FeedbackBook feedbackManager, String courseID, T feedbackData, String feedbackType, String date) {
        feedbackManager.addFeedback(this.getEmail(), courseID, feedbackData, feedbackType, date);
    }
}
